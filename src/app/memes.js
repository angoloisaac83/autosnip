  ...
  return (
    <>
      ...
      <h1 className="text-3xl font-bold mb-8 text-white">
        ...
        <label class="flex items-center gap-2 cursor-pointer font-medium group"><input type="checkbox" class="h-4 w-4 accent-highlight flex-shrink-0" /> <span class="text-grey1 text-[8px] group-hover:brightness-125 group-active:brightness-75 select-none">Freeze Auth Disabled</span></label></div></div> <button data-intro="filterRug_mobile" id="filterRug_mobile" class="font-bold text-grey1 hover:brightness-125 active:brightness-75 mt-4 text-sm lg:mt-0">+ All Filters</button></div> <button class="text-grey3 text-xs py-1 px-2 hover:text-grey2 active:text-grey4 top-1 absolute lg:static right-1"><span class="hidden lg:block text-lg">HIDE</span> <span class="lg:hidden text-2xl leading-none">×</span></button></div></div>
        {/* {connectedWallet && (
          <span className="ml-4 text-sm font-normal bg-green-900 px-3 py-1 rounded-full">
            Connected: {connectedWallet} ({walletAddress.slice(0, 6)}...{walletAddress.slice(-4)})
          </span>
