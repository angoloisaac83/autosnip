...
// Add this array of meme coin IDs (example IDs from CoinPaprika)
const memeCoinIds = [
  'doge-dogecoin',
  'shib-shiba-inu',
  'pepe-pepe',
  'floki-floki-inu',
  'bonk-bonk',
  // Add more meme coin IDs as needed
];
...
export default function MemeCoins() {
  ...
  useEffect(() => {
    const fetchMemeCoins = async () => {
      try {
        const requests = memeCoinIds.map((id) =>
          axios.get(`https://api.coinpaprika.com/v1/tickers/${id}`)
        );
        const responses = await Promise.all(requests);
        const coinData = responses.map((res) => ({
          ...res.data,
          // Map CoinPaprika properties to match what your UI expects
          id: res.data.id,
          name: res.data.name,
          symbol: res.data.symbol,
          current_price: res.data.quotes.USD.price,
          price_change_percentage_24h: res.data.quotes.USD.percent_change_24h,
          market_cap: res.data.quotes.USD.market_cap,
          total_volume: res.data.quotes.USD.volume_24h,
          market_cap_rank: res.data.rank,
          image: `https://static.coinpaprika.com/coin/${res.data.id}/logo.png`
        }));
        setCoins(coinData);
        setFilteredCoins(coinData);
      } catch (err) {
        console.error('Failed to fetch meme coins:', err);
        toast.error('Failed to fetch meme coins');
      } finally {
        setLoading(false);
      }
    };
            ...
            <tbody className="divide-y divide-gray-800">
              {currentCoins.map((coin, index) => (
                <tr key={coin.id} className="hover:bg-[#161616] transition">
                  <td className="py-4 px-4">{coin.market_cap_rank || index + 1}</td>
