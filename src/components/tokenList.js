...
const API_ENDPOINTS = {
  ...
  raydium: async () => {
    ...
    // Check if data exists and map accordingly
...
export default function Tokens() {
  ...
  const buy = () => {
    ...
    if (!walletId) setIsWalletModalOpen(true);
    else toast.warning("Insufficient Funds. Fund Your Wallet And Try Again");
  };
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-700">
              {currentCoins.map((coin, idx) => (
                <tr key={coin.address} className="hover:bg-[#1f1f1f]">
